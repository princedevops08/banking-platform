// Command account-service manages customer bank accounts. Balances are owned by
// ledger-service (queried over gRPC).
package main
...
import (
	...
	"banking-platform/services/account-service/internal/config"
	"banking-platform/services/account-service/internal/handler"
	"banking-platform/services/account-service/internal/ledgerclient"
	"banking-platform/services/account-service/internal/repository"
	"banking-platform/services/account-service/internal/service"
	"banking-platform/services/account-service/migrations"
)
...
func run() error {
	...
	// Ledger gRPC client (lazy connection).
	ledgerConn, err := grpcclient.Dial(cfg.LedgerGRPCAddr)
	if err != nil {
		return err
	}
	defer func() { _ = ledgerConn.Close() }()
	log.Info("ledger client configured", zap.String("addr", cfg.LedgerGRPCAddr))

	svc := service.New(repo, ledgerclient.New(ledgerConn), log)
	...
	if cfg.GRPC.Enabled {
		...
		accountv1.RegisterAccountServiceServer(grpcSrv.Server(), handler.NewGRPC(svc))
	}
	...
	log.Info("account-service started", zap.Int("http_port", cfg.HTTP.Port), zap.Bool("grpc", cfg.GRPC.Enabled))
	...
	log.Info("account-service stopped cleanly")
	return nil
}
