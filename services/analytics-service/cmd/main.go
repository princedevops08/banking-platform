// Command analytics-service consumes transaction events from Kafka and maintains
// per-transaction-type aggregates, exposing a staff-only summary API.
package main
...
import (
	...
	"banking-platform/services/analytics-service/internal/config"
	"banking-platform/services/analytics-service/internal/handler"
	...
	"banking-platform/services/analytics-service/migrations"
)

// consumedTopics are the domain event streams analytics-service aggregates.
var consumedTopics = []string{"banking.transactions"}
...
func run() error {
	...
	// Kafka consumers (one per topic, same consumer group = this service).
	var consumers []*kafka.Consumer
	if cfg.Kafka.Enabled {
		for _, topic := range consumedTopics {
			c := kafka.NewConsumer(cfg.Kafka.Brokers, cfg.ServiceName, topic)
			consumers = append(consumers, c)
			go func(c *kafka.Consumer) {
				if err := c.Run(ctx, svc.Handle); err != nil {
					log.Error("consumer stopped", zap.Error(err))
				}
			}(c)
		}
		log.Info("kafka consumers started", zap.Strings("topics", consumedTopics))
	}
	...
	handler.NewHTTP(svc).Register(srv.Router(), tokenMgr.Middleware(), auth.RequireRole("admin", "teller"))
	checker.SetReady(true)

	errCh := make(chan error, 1)
	go func() { errCh <- srv.Start() }()
	log.Info("analytics-service started", zap.Int("http_port", cfg.HTTP.Port))
	...
	log.Info("analytics-service stopped cleanly")
	return nil
}
