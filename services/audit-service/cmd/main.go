// Command audit-service consumes all domain events from Kafka and records them
// in an append-only audit log, exposing a staff-only query API.
package main
...
import (
	...
	"banking-platform/services/audit-service/internal/config"
	"banking-platform/services/audit-service/internal/handler"
	"banking-platform/services/audit-service/internal/repository"
	"banking-platform/services/audit-service/internal/service"
	"banking-platform/services/audit-service/migrations"
)

// consumedTopics are the domain event streams audit-service records.
var consumedTopics = []string{"banking.transactions", "banking.payments", "banking.wallet"}
...
func run() error {
	...
	log.Info("audit-service started", zap.Int("http_port", cfg.HTTP.Port))
	...
	log.Info("audit-service stopped cleanly")
	return nil
}
