// Command authz-service is the platform's central RBAC authorization service
// (REST admin + gRPC Check).
package main
...
import (
	...
	"banking-platform/services/authz-service/internal/config"
	"banking-platform/services/authz-service/internal/handler"
	"banking-platform/services/authz-service/internal/repository"
	"banking-platform/services/authz-service/internal/service"
	"banking-platform/services/authz-service/migrations"
)
...
func run() error {
	...
	handler.NewHTTP(svc).Register(srv.Router(), tokenMgr.Middleware(), auth.RequireRole("admin"))
	checker.SetReady(true)

	var grpcSrv *grpcserver.Server
	if cfg.GRPC.Enabled {
		grpcSrv = grpcserver.New(grpcserver.Config{Host: cfg.GRPC.Host, Port: cfg.GRPC.Port}, log)
		authzv1.RegisterAuthzServiceServer(grpcSrv.Server(), handler.NewGRPC(svc))
	}
	...
	log.Info("authz-service started", zap.Int("http_port", cfg.HTTP.Port), zap.Bool("grpc", cfg.GRPC.Enabled))
	...
	log.Info("authz-service stopped cleanly")
	return nil
}
