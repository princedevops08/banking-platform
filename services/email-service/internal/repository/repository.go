// Package repository provides persistence for email-service.
package repository
...
// Repository stores email messages.
...
// Postgres persists messages under the email_ prefix.
...
const cols = "id, to_addr, subject, body, status, created_at"

func (r *Postgres) Create(ctx context.Context, m domain.Message) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO email_messages (id, to_addr, subject, body, status, created_at)
		 VALUES ($1,$2,$3,$4,$5,$6)`,
		m.ID, m.To, m.Subject, m.Body, m.Status, m.CreatedAt)
	return err
}
...
func (r *Postgres) List(ctx context.Context, limit int) ([]domain.Message, error) {
	...
	rows, err := r.pool.Query(ctx,
		`SELECT `+cols+` FROM email_messages ORDER BY created_at DESC LIMIT $1`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.To, &m.Subject, &m.Body, &m.Status, &m.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, m)
	}
