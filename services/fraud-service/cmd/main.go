// Command fraud-service consumes transaction and payment events from Kafka,
// applies simple fraud rules, and exposes a staff-only alert query API.
package main
...
import (
	...
	"banking-platform/services/fraud-service/internal/config"
	"banking-platform/services/fraud-service/internal/handler"
	"banking-platform/services/fraud-service/internal/repository"
	"banking-platform/services/fraud-service/internal/service"
	"banking-platform/services/fraud-service/migrations"
)

// consumedTopics are the domain event streams fraud-service inspects.
var consumedTopics = []string{"banking.transactions", "banking.payments"}
...
func run() error {
	...
	log.Info("fraud-service started", zap.Int("http_port", cfg.HTTP.Port))
	...
	log.Info("fraud-service stopped cleanly")
	return nil
}
