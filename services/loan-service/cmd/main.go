// Command loan-service manages loan applications, approval and disbursement.
package main
...
import (
	...
	"banking-platform/services/loan-service/internal/config"
	"banking-platform/services/loan-service/internal/handler"
	...
	"banking-platform/services/loan-service/migrations"
)
...
func run() error {
	...
	handler.NewHTTP(svc).Register(srv.Router(), tokenMgr.Middleware(), auth.RequireRole("teller", "admin"))
	checker.SetReady(true)

	errCh := make(chan error, 1)
	go func() { errCh <- srv.Start() }()
	log.Info("loan-service started", zap.Int("http_port", cfg.HTTP.Port))
	...
	log.Info("loan-service stopped cleanly")
	return nil
}
