// Command payment-service executes outbound payments to beneficiaries, posting
// each to the ledger and emitting events via the transactional outbox.
package main
...
import (
	...
	"banking-platform/services/payment-service/internal/config"
	"banking-platform/services/payment-service/internal/handler"
	...
	"banking-platform/services/payment-service/migrations"
)
...
func run() error {
	...
	if cfg.Postgres.Enabled {
		...
		if cfg.Kafka.Enabled {
			...
			go outbox.NewRelay(pool, producer, cfg.ServiceName, log).Run(ctx)
	...
	log.Info("payment-service started", zap.Int("http_port", cfg.HTTP.Port))
