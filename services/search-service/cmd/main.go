// Command search-service consumes transaction events from Kafka, builds a
// searchable index, and exposes an authenticated query API.
package main
...
import (
	...
	"banking-platform/services/search-service/internal/config"
	"banking-platform/services/search-service/internal/handler"
	"banking-platform/services/search-service/internal/repository"
	"banking-platform/services/search-service/internal/service"
	"banking-platform/services/search-service/migrations"
)

// consumedTopics are the domain event streams search-service indexes.
...
func run() error {
	...
	log.Info("search-service started", zap.Int("http_port", cfg.HTTP.Port))
