// Command statement-service consumes transaction events from Kafka and derives
// per-account statement entries, exposing an authenticated query API.
package main
...
import (
	...
	"banking-platform/services/statement-service/internal/config"
	"banking-platform/services/statement-service/internal/handler"
	...
	"banking-platform/services/statement-service/migrations"
)

// consumedTopics are the domain event streams statement-service records.
...
func run() error {
	...
	log.Info("statement-service started", zap.Int("http_port", cfg.HTTP.Port))
	...
	log.Info("statement-service stopped cleanly")
	return nil
}
