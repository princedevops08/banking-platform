// Command transaction-service records deposits, withdrawals and transfers,
// posting each as a balanced ledger transaction and emitting events via the
...
import (
	...
	"banking-platform/pkg/grpcclient"
	"banking-platform/pkg/health"
	"banking-platform/pkg/httpserver"
	pkgkafka "banking-platform/pkg/kafka"
	...
	"banking-platform/services/transaction-service/internal/config"
	"banking-platform/services/transaction-service/internal/handler"
	...
	"banking-platform/services/transaction-service/migrations"
)
...
func run() error {
	...
	if cfg.Postgres.Enabled {
		...
		// Outbox relay publishes transaction events to Kafka when enabled.
		if cfg.Kafka.Enabled {
			producer := pkgkafka.NewProducer(cfg.Kafka.Brokers)
			defer func() { _ = producer.Close() }()
			relay := outbox.NewRelay(pool, producer, cfg.ServiceName, log)
			go relay.Run(ctx)
			log.Info("outbox relay started", zap.Strings("brokers", cfg.Kafka.Brokers))
		}
	}

	ledgerConn, err := grpcclient.Dial(cfg.LedgerGRPCAddr)
	if err != nil {
		return err
	}
	defer func() { _ = ledgerConn.Close() }()

	svc := service.New(repo, ledgerclient.New(ledgerConn), cfg.ServiceName, emitEvents, log)
	...
	log.Info("transaction-service started", zap.Int("http_port", cfg.HTTP.Port))
	...
	log.Info("transaction-service stopped cleanly")
	return nil
}
